...
//Handle gameplay utility updates from external source
    //Expects a list of Objects specifying the utility to update and the action to take
//Option to handle/trigger more than one utility within a single message instance
function handleExternalUtilityUpdates(utility_update_objs) 
{
    for (i = 0; i < utility_update_objs.length; i++)
    {
        if (utility_update_objs[i].utility == 'pause_game_flag'){
            togglePauseGame();
            console.log("opponent paiused videogame");
        }
        //else if other non-gameplay essential utility flags...
    }
